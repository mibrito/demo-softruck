				...
				Cities.findOne(citiesRef[selMunicipio])
					.then(function(cityFound){
						if(!cityFound){
							...
							return newCity.save()
								.then(function(cityInserted){
									// add city on state
									state.document.cities.push(cityInserted);
...
var crawlStations = function(selSemana, fuel, city){
	...
	return request({
		...
		rows.slice(1, rows.length).each(function(i, row){
			...
			allInsertions.push(
				Stations.findOne(stations[name])	// findOrCreate
						...
						if(!stationFound){
							...
							return newStation.save()
								.then(function(stationSaved){
									city.document.stations.push(stationSaved);
									return city.document.save()
										.then(function(){
											return Promise.resolve(stationSaved);
										});
								});
						}else{
